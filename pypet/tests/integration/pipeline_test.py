...
class Multiply(object):
    ...
    def __call__(self, traj, i, w=0):
        if traj.idx == 0:
            ...
            time.sleep(0.2)
        z = traj.x * traj.y + i + w
        zres = traj.add_result('z', z)
        g=traj.res.add_group('I.link.to.$')
        g.add_link('z', zres)
        if 'jjj.kkk' not in traj:
            h = traj.res.add_group('jjj.kkk')
        else:
            h = traj.jjj.kkk
        h.add_link('$', zres)
        return z
...
def postproc(traj, results, idx):
    ...
    if isinstance(traj.storage_service, (LockWrapper, ReferenceWrapper)):
        traj.load_skeleton()

    if isinstance(traj.storage_service, (QueueStorageServiceSender, PipeStorageServiceSender)):
        try:
            traj.load()
...
def mypipelin_with_iter_args(traj):
    ...
    traj.add_parameter('x', 1, comment='1st')
    traj.add_parameter('y', 1, comment='1st')

    exp_dict = {'x':[1, 2, 3, 4],
                     'y':[1, 2, 3, 4]}

    traj.explore(exp_dict)
...
class TestPostProc(TrajectoryComparator):
    ...
    def test_postprocessing(self):
        ...
        traj1.add_result('test.run_00000000.f', 555)
        traj2.add_result('test.run_00000000.f', 555)
        traj1.add_link('linking', traj1.get('f'))
        traj2.add_link('linking', traj2.get('f'))

        for traj in trajs:
            traj.add_parameter('x', 1, comment='1st')
            traj.add_parameter('y', 1, comment='2nd')

        exp_dict2 = {'x':[1, 2, 3, 4, 1, 2, 2, 3],
                     'y':[1, 2, 3, 4, 1, 2, 0, 1]}

        traj2.explore(exp_dict2)

        exp_dict1 = {'x':[1, 2, 3, 4],
                     'y':[1, 2, 3, 4]}

        traj1.explore(exp_dict1)

        env2.run(Multiply(), 22)

        env1.add_postprocessing(postproc, 42)

        env1.run(Multiply(), 22)

        traj1.load(load_data=2)
    ...
    def test_postprocessing_iter_args(self):
        ...
        env2.run_map(Multiply(), [22,23,24,25,5,6,7,8], w=[5,6,7,8,7,8,9,10])

        env1.add_postprocessing(postproc_with_iter_args, 42)

        env1.run_map(Multiply(), [22,23,24,25], w=[5,6,7,8])
    ...
    def test_pipeline(self):
        ...
        traj2.add_parameter('x', 1, comment='1st')
        traj2.add_parameter('y', 1, comment='1st')
        ...
        res2 = env2.run(Multiply(), 22)

        self.are_results_in_order(res2)

        traj_name = traj1.name
    ...
    def test_pipeline_with_iter_args(self):
        ...
        res2 = env2.run_map(Multiply(), [22,23,24,25,5,6,7,8], w=[5,6,7,8,7,8,9,10])
